...
SemerError::SemerError(
    ...
    std::string message,
    std::string hint
) : type(type), level(level), metadata(metadata), sourceCode(sourceCode),
    message(std::move(message)), hint(std::move(hint)) {};
...
const std::string SemerError::toString() const {
    ...
    for (size_t line = this->metadata.start.line; line <= this->metadata.end.line; ++line) {
        ...
        if (line == this->metadata.start.line) {
            ...
            result += std::string(this->metadata.start.column + 3, ' ') + COLOR + std::string(length,
                      '~') + RESET;
...
Semer::Semer(const std::string& sourceCode, const Program& program) : sourceCode(sourceCode),
    program(program) {};
        ...
        if constexpr(std::is_same_v<ExprType, StringLiteral>) {
            ...
            if (optLeft.has_value() && optRight.has_value()) {
                ...
                std::visit([&e, &type](const auto & left, const auto & right) {
                    ...
                    if (left->compare(std::make_shared<IntegerType>()) &&
                        right->compare(std::make_shared<IntegerType>())) {
                        if (e.op == "/") {
                            type = std::make_shared<FloatType>();
                        } else {
                            type = std::make_shared<IntegerType>();
                        }
                    } else if ((left->compare(std::make_shared<IntegerType>()) &&
                                right->compare(std::make_shared<FloatType>())) ||
                        ...
                        type = std::make_shared<FloatType>();
                    } else if (e.op == "+" && left->compare(std::make_shared<StringType>()) &&
            ...
            if constexpr(this->isLiteral(left) && this->isLiteral(right)) {
                ...
                !(n.op == "&&" || n.op == "||")
                    ...
                    this->errors.push_back(SemerError(
                                               ...
                                               n.metadata,
                                               this->sourceCode,
                                               "Can only perform '&&' and '||' operations between string and '" +
                                               this->resolveExpressionReturnType(right, scope).value()->toString() + "'.",
                                               "Please use a valid operator for strings."
                                           ));
                } else if constexpr(
                    this->isNumber(left) &&
                ...
                !(n.op == "&&" || n.op == "||")
                    ...
                    this->errors.push_back(SemerError(
                                               ...
                                               n.metadata,
                                               this->sourceCode,
                                               "Can only perform '&&' and '||' operations between number and '" +
                                               this->resolveExpressionReturnType(right, scope).value()->toString() + "'.",
                    ...
                    this->errors.push_back(SemerError(
                                               ...
                                               n.metadata,
                                               this->sourceCode,
                                               "Can only perform '&&' and '||' operations between bool and '" + this->resolveExpressionReturnType(
                                                   right, scope).value()->toString() + "'.",
                ...
                std::visit([&](const auto & leftType, const auto & rightType) {
                    ...
                    } else if constexpr(leftType->compare(std::make_shared<BooleanType>()) && !(op == "&&" ||
                    op == "||")) {
                        // * Booleans can only perform '&&' and '||' operations with booleans and other types
    ...
    if constexpr(std::is_same_v<T, VariableDeclaration>) {
        ...
        if (!n.value.has_value()) {
            ...
            this->errors.push_back(SemerError(
                                       ...
                                       n.metadata,
                                       this->sourceCode,
                                       "'" + n.identifier + "' is defined as a " + word +
                                       " variable but has no initialization value. This will result in undefined behavior.",
            ...
            std::visit([&](auto&& type, const auto & expr) {
                ...
                if (exprType.has_value() && !type->compare(exprType.value())) {
                    ...
                    this->errors.push_back(SemerError(
                                               ...
                                               "Either change the type of the variable to '" + exprTypeString + "' or change the value to type '" +
                                               typeString + "'."
                                           ));
                }
