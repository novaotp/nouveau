...
Parser::Parser(std::string sourceCode, std::vector<Token> tokens) : sourceCode(sourceCode),
    tokens(tokens) {};
...
const Token& Parser::expectToken(const TokenType& expected, std::string hint = "") {
    ...
    if (currentToken.type != expected) {
        throw std::runtime_error(
            ...
            + "\n\n\t" + std::to_string(currentToken.metadata.line) + " | " + splitStringByNewline(
                this->sourceCode).at(currentToken.metadata.line - 1)
            + "\n\t" + std::string(currentToken.metadata.column + 3,
                                   ' ') + RED + std::string(currentToken.metadata.length, '~')
    ...
    if (currentToken.value == "int") {
        ...
        throw std::runtime_error(
            ...
            + "\n\n\t" + std::to_string(currentToken.metadata.line) + " | " + splitStringByNewline(
                this->sourceCode).at(currentToken.metadata.line - 1)
            + "\n\t" + std::string(currentToken.metadata.column + 3,
                                   ' ') + std::string(currentToken.metadata.length, '~')
...
Program Parser::parse() {
    ...
    while (this->getCurrentToken().type != TokenType::END_OF_FILE) {
        std::variant<Statement, Expression, std::monostate> statementOrExpression =
            this->parseStatementOrExpression();
...
VariableDeclaration Parser::parseVariableDeclaration() {
    ...
    std::string identifier = this->expectToken(TokenType::IDENTIFIER,
                                               "Did you forget to set a name for your variable ? ").value;

    if (this->getCurrentToken().type != TokenType::ASSIGNMENT_OPERATOR) {
        NodePosition end = this->getCurrentToken().metadata.toEndPosition();

        this->expectToken(TokenType::SEMI_COLON,
                          "A variable declaration must end with a ';'. Did you forget it ?"); // Skip the ";" token
    ...
    return VariableDeclaration(NodeMetadata(start, end), isMutable, type, identifier,
...
VariableAssignment Parser::parseVariableAssignment() {
    ...
    std::string identifier = this->expectToken(TokenType::IDENTIFIER,
                                               "Assignment only works on a variable.").value;
    ...
    return VariableAssignment(NodeMetadata(start, end), identifier, op,
                              std::make_shared<Expression>(std::move(value)));
}
...
Expression Parser::parsePrimitiveExpression() {
    ...
    switch (currentToken.type) {
        ...
        case TokenType::LEFT_PARENTHESIS: {
            ...
            this->expectToken(TokenType::RIGHT_PARENTHESIS,
                              "Did you forget to close the parenthesis ?"); // Skip the ")" token
