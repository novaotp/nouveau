...
StringLiteral::StringLiteral(NodeMetadata metadata, std::string value) : metadata(metadata),
    value(value) {};

IntLiteral::IntLiteral(NodeMetadata metadata, int value) : metadata(metadata), value(value) {};

FloatLiteral::FloatLiteral(NodeMetadata metadata, float value) : metadata(metadata),
    value(value) {};

BooleanLiteral::BooleanLiteral(NodeMetadata metadata, bool value) : metadata(metadata),
    value(value) {};

Identifier::Identifier(NodeMetadata metadata, std::string name) : metadata(metadata), name(name) {};

LogicalNotOperation::LogicalNotOperation(NodeMetadata metadata,
                                         std::shared_ptr<Expression> expression)
    : metadata(metadata), expression(std::move(expression)) {};

BinaryOperation::BinaryOperation(NodeMetadata metadata, std::shared_ptr<Expression> left,
...
void printStatement(const NodeType& n, const size_t indentCount) {
    ...
    if constexpr(std::is_same_v<NodeType, VariableDeclaration>) {
        ...
        std::cout << (indent + std::string(SPACE_COUNT,
                                           ' ')) << "IsMutable: " << (n.isMutable ? "true" : "false") << std::endl;
        ...
        std::cout << (indent + std::string(SPACE_COUNT,
                                           ' ')) << "Identifier: " << n.identifier << std::endl;
