...
namespace Catch {
    ...
    Clara::Parser makeCommandLineParser(ConfigData& config) {
        ...
        auto cli = ExeName(config.processName) | Help(config.showHelp) | Opt(config.showSuccessfulTests)["-s"]["--success"]("include successful tests in output") | Opt(config.shouldDebugBreak)["-b"]["--break"]("break into debugger on failure") | Opt(config.noThrow)["-e"]["--nothrow"]("skip exception tests") | Opt(config.showInvisibles)["-i"]["--invisibles"]("show invisibles (tabs, newlines)") | Opt(config.defaultOutputFilename, "filename")["-o"]["--out"]("default output filename") | Opt(accept_many, setReporter, "name[::key=value]*")["-r"]["--reporter"]("reporter to use (defaults to console)") | Opt(config.name, "name")["-n"]["--name"]("suite name") | Opt([&](bool) { config.abortAfter = 1; })["-a"]["--abort"]("abort at first failure") |
...
namespace Catch {
    ...
    namespace {
        ...
        class RedirectedStreamNew {
            ...
            RedirectedStreamNew(std::ostream& originalStream,
                std::ostream& redirectionStream) : m_originalStream(originalStream),
        ...
        class StreamRedirect : public OutputRedirect {
            ...
            public:
            StreamRedirect() : m_cout(Catch::cout(), m_redirectedOut.get()),
        ...
        class FileRedirect : public OutputRedirect {
            ...
            public:
            FileRedirect() : m_originalOut(dup(fileno(stdout))),
    ...
    RedirectGuard::RedirectGuard(bool activate, OutputRedirect& redirectImpl) : m_redirect(&redirectImpl),
        m_activate(activate),
    ...
    RedirectGuard::RedirectGuard(RedirectGuard&& rhs) noexcept : m_redirect(rhs.m_redirect),
